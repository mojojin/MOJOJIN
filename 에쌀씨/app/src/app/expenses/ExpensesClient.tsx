    ...
    <div className="min-h-screen bg-white pb-24 font-sans text-gray-900">
      ...
      <div className="max-w-lg mx-auto px-4 space-y-6 pt-5">
        ...
        <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-150">
          <button
            onClick={() => setExpenseTab('expenses')}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-3">
            ...
            <span className="text-xs text-gray-500 font-bold">로딩 중...</span>
          </div>
        ) : (
          <>
            {expenseTab === 'expenses' ? (
                      ...
                      {canViewBalance ? (
                        <div className="pt-4 border-t border-gray-800 grid grid-cols-3 gap-2 text-center text-[10px] text-gray-400">
                          ...
                          <div>
                            ...
                            <p className="text-xs font-bold text-neon-yellow mt-1">₩{currentBalance.toLocaleString()}</p>
                          </div>
                        </div>
                      ) : (
                    ...
                    {expenses.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {paginatedExpenses.map((expense) => (
                          <div
                            key={expense.id}
                            className="bg-white rounded-xl border border-gray-200 py-2.5 px-4 flex items-center justify-between hover:bg-gray-55 transition-all text-xs"
                          >
                            <div className="flex items-center gap-3">
                        ...
                        {totalPages > 1 && (
                          <div className="flex justify-between items-center pt-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-bold text-gray-500 hover:bg-gray-55 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                            >
                              이전
                            </button>
                          ...
                          <div key={p.id} className="flex justify-between items-center py-2 text-xs first:pt-0 last:pb-0">
                            ...
                            <div>
                              {isExempted ? (
                                ...
                                <span className="text-orange-650 font-bold bg-orange-50 border border-orange-100 px-2.5 py-0.5 rounded-full text-[10px]">승인대기</span>
                              ) : (
                                <span className="text-red-500 font-bold bg-red-50 border border-red-100 px-2.5 py-0.5 rounded-full text-[10px]">미납</span>
                              )}
                            </div>
                          </div>
                        )
