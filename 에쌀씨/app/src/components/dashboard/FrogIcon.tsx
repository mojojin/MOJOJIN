...
export default function FrogIcon({ km, className = '', size = 'md' }: FrogIconProps) {
  ...
  return (
    <svg 
      viewBox="0 0 100 100" 
      className={`${sizeClasses[size]} ${className} ${tier.pulse ? 'animate-pulse' : ''} transition-all duration-300 inline-block rounded-full bg-white/5`}
      style={{
        boxShadow: tier.glow ? `0 0 12px ${tier.glow}` : 'none'
      }}
    >
      <defs>
