        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {paginatedItems.map(item => {
              ...
              return (
                <div key={item.id} className="rounded-xl border border-gray-200 bg-white p-2 md:p-2.5 flex flex-col justify-between shadow-sm hover:border-gray-300 transition-all">
                  <div>
                    <div className="flex justify-between items-start gap-1.5 mb-0.5">
                      <h3 className="text-[12.5px] font-extrabold text-gray-900 break-all">{item.item_name}</h3>
                      <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${cond.c}`}>{cond.t}</span>
                    </div>
                    {item.notes && (
                      <p className="text-[9.5px] text-gray-500 break-all line-clamp-2 mt-0.5">
                        📝 {item.notes}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex justify-between items-center mt-2 pt-1.5 border-t border-gray-100">
                    <div className="text-[9.5px] text-gray-550">
                      <span>수량: <strong className="text-gray-900 font-mono font-bold">{item.quantity}</strong>개</span>
                      <span className="mx-1 text-gray-300">·</span>
                      <span>보관: <strong className="text-gray-900 font-bold">{item.manager_name || '없음'}</strong></span>
                    </div>

                    <div className="flex gap-1">
                      <button onClick={() => openForm(item)} className="px-1.5 py-0.5 bg-gray-50 border border-gray-200 rounded-md text-[9px] font-bold text-gray-500 hover:text-gray-950 transition-all active:scale-95 flex items-center gap-0.5">
                        <svg className="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                        수정
                      </button>
                      <button onClick={() => handleDelete(item.id)} className="px-1.5 py-0.5 bg-red-50 border border-red-150 rounded-md text-[9px] font-bold text-red-650 hover:bg-red-100 transition-all active:scale-95 flex items-center gap-0.5">
                        <svg className="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
